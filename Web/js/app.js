...
// ==================== 存储管理 ====================
const STORAGE_KEY = 'mayuan_question_bank';
...
function resetAll() {
    ...
    localStorage.removeItem('mayuan_expanded');
    APP.progress = {};
...
function renderHome() {
    ...
    let html = `
        <div class="chapter-list-header">
            <img src="public/logo-site.svg" alt="马原题库" class="hero-logo">
            <h1>马原题库练习</h1>
            <p>马克思主义基本原理 · 共 ${QUESTION_BANK.chapters.length} 章 · ${total} 题</p>
        </div>
...
// ==================== 主题管理 ====================
const THEME_KEY = 'mayuan_theme';
const SYSTEM_DARK_QUERY = '(prefers-color-scheme: dark)';

function getSystemTheme() {
    return window.matchMedia && window.matchMedia(SYSTEM_DARK_QUERY).matches ? 'dark' : 'light';
}

function applyThemeClass(theme) {
    document.body.dataset.theme = theme;
    document.body.classList.remove('dark', 'light');
    document.body.classList.add(theme === 'auto' ? getSystemTheme() : theme);
}
...
function initTheme() {
    ...
    const activeTheme = saved === 'dark' || saved === 'light' ? saved : 'auto';
...
if (window.matchMedia) {
    window.matchMedia(SYSTEM_DARK_QUERY).addEventListener('change', () => {
        const saved = localStorage.getItem(THEME_KEY);
        if (!saved || saved === 'auto') {
            animateThemeChange(() => applyThemeClass('auto'));
        }
    });
}
...
function init() {
    ...
    initTheme();
    try {
        const expRaw = localStorage.getItem('mayuan_expanded');
...
function saveExpandedState() {
    localStorage.setItem('mayuan_expanded', JSON.stringify([...APP.expandedExplanations]));
}
